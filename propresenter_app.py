...
def _maybe_advance_from_pp(state: dict) -> dict:
    """Auto-track ProPresenter: poll /v1/timers/current and /v1/presentation/active
    to keep current_index in sync with whatever is actually happening in PP.
    Returns the (possibly mutated) state. Manual cue overrides are honoured for
    a 10-second window via state['manual_override_until']."""
    import requests as req
    ...
    # 1) Try running [RB] timer
    try:
        r = req.get(f"{base}/v1/timers/current", timeout=2)
        if r.ok:
            running = r.json() or []
            # Response shape: list of {"id": {"name": ..., "uuid": ...},
            #                          "time": "00:04:32", "state": "running"}
            for t in running if isinstance(running, list) else []:
                ...
                if tstate not in ("running", "started", "active"):
                    continue
                # Find the item whose pp_timer_name matches
                for i, it in enumerate(items):
                    if (it.get("pp_timer_name") or "") == tname:
                        ...
                        # Use timer's reported remaining for accuracy
    ...
    # 2) Fall back to active presentation name match
    try:
        ...
        if r.ok:
            ...
            if active_name:
                ...
                # rapidfuzz might not be available; use difflib (already imported)
