...
SERVICE_MATE_CUE_ADDENDUM = (
    ...
    "For EACH item, also include a `cues` object with three keys — "
    "`screen`, `sound`, `lights`. Each key holds an ARRAY of 1-3 short "
    "imperative phrases (≤ 55 chars each) telling the operator at that "
    "station what to do while this item is current. The clock rotates "
    "through them, five seconds each, so give each station every distinct "
    "thing it needs — not one line crammed with several.\n"
    "  - cues.screen  — what the SCREEN/lyric op should cue\n"
    "  - cues.sound   — what the SOUND op should do (which mics on/off)\n"
    "  - cues.lights  — what the LIGHTS op should do\n"
    "Read the notes for specifics and turn anything station-relevant into "
    "its own cue for THAT station. A video in the notes matters to screen "
    "AND sound; a panel discussion means several mics; a solo means one.\n"
    "Example, for an MC welcome whose notes mention a trailer video:\n"
    "  cues.screen = [\"Slide — Welcome\", \"Trailer video ready\"]\n"
    "  cues.sound  = [\"Mics on for Kiara & Annie\", "
